...
def getPredsFromFilelist(model,filelist,file_dir,file_ext,
	...
	for i in range(len(filepaths)):
		...
		# padded_data = np.concatenate((pad_top,data),axis=0)
		# padded_data = np.concatenate((padded_data,pad_bot),axis=0)
		# print "before",padded_data.shape
		# data = []
		...
		for x in range(len(preds)):
			#prxnt x,"/",len(preds)
			preds[x]=np.delete(preds[x],len(preds[x])-1)
	        preds[x]=[float(j)/sum(preds[x]) for j in preds[x]]
...
getPredsFromFilelist(model,"wsj0_dev.fileids","feat_ci_dev_mls/",".mfc","test_senones/",".sen",1)
