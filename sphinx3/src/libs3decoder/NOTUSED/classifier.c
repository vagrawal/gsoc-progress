        ...
        float64 _mix_weight_floor, char *_gm_type, int _post_classify)
{
    ...
    _cl->gmm =
        mgau_init(_means_file, _vars_file, _var_floor, _mix_weights_file,
...
cl_calc_frame_gmm(classifier_t * _cl, float32 * _frame)
{
    ...
    for (class_index = gmm->n_mgau - 1; class_index >= 0; class_index--) {
        ...
        for (gau_index = gmm->mgau->n_comp - 1; gau_index >= 0;
             gau_index--) {
            adjust = gmm->mgau[class_index].lrd[gau_index];
            dsum = 0.0;
            for (i = 0; i < CEP_LEN; i++) {
                diff =
                    _frame[i] - gmm->mgau[class_index].mean[gau_index][i];
                dsum +=
                    diff * diff * gmm->mgau[class_index].var[gau_index][i];
            }
            _cl->frame_gmm[class_index] +=
                gmm->mgau[class_index].mixw_f[gau_index] * adjust *
                exp(dsum * -0.5);
        }
    }
}
                   ...
                   int **_classes)
{
    ...
    for (frame_index = 0; frame_index < _num_frames; frame_index++) {
        ...
        best_class = -1;
        best_prob = 0.0f;
        for (class_index = NUM_CLASSES - 1; class_index >= 0;
             class_index--) {
            prob =
                _cl->frame_gmm[class_index] *
                _cl->class_prior_prob[class_index];
            if (best_prob < prob) {
                best_prob = prob;
